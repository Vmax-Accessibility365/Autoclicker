...
class MainActivity : AppCompatActivity() {
    ...
    private fun onStartClicked() {
        val targetText = binding.textInput.text.toString().trim()
        if (TextUtils.isEmpty(targetText)) {
            Toast.makeText(this, "Pehle target text bharein.", Toast.LENGTH_SHORT).show()
            return
        }

        val interval = binding.intervalInput.text.toString().toLongOrNull()
        if (interval == null || interval < 200) {
            Toast.makeText(this, "Interval kam se kam 200ms hona chahiye.", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU &&
            ...
            // User can tap START again after granting; we still proceed below
            // since the foreground service itself does not require this
            // permission to function, only to show its status notification.
        }

        // Save inputs before requesting capture, so we don't lose them.
    ...
    private var pendingInterval: Long = 1000L
    private var pendingRetry: Boolean = true

    private fun launchAutoClickService(resultCode: Int, data: Intent) {
        val intent = Intent(this, AutoClickService::class.java).apply {
            ...
            putExtra(AutoClickService.EXTRA_TARGET_TEXT, pendingTargetText)
            putExtra(AutoClickService.EXTRA_INTERVAL_MS, pendingInterval)
            putExtra(AutoClickService.EXTRA_RETRY, pendingRetry)
        }
    ...
    private fun updateStatusUi(running: Boolean = AutoClickService.isRunning) {
    ...
    private fun isAccessibilityServiceEnabled(): Boolean {
        val expectedComponent = ComponentName(this, ClickAccessibilityService::class.java)
